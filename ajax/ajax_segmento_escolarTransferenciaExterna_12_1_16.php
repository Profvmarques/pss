...
if ($_POST['idtipo_segmento'] == 4) {
 ...
 if ($_POST['idtipo_segmento'] == 1) {
    ...
    $se->consultar("select * from vagas v inner join segmentoescolar se on(se.idsegmento_escolar=v.idsegmento_escolar) where v.idunidade_escolar=".$_SESSION['idunidade_escolar']." and v.idsegmento_escolar in(6,7,8,9,10,11,12,13,14) and v.tipovaga='TRANSFERENCIA_EXTERNA'");
    $linha = $se->Linha;
    $resultado = $se->Result;
}
 ...
 if ($_POST['idtipo_segmento'] == 2) {
    ...
    $se->consultar("select * from vagas v inner join segmentoescolar se on(se.idsegmento_escolar=v.idsegmento_escolar) where v.idunidade_escolar=".$_SESSION['idunidade_escolar']." and v.idsegmento_escolar in(1,2,3,4,5) and v.tipovaga='TRANSFERENCIA_EXTERNA'");
    $linha = $se->Linha;
    $resultado = $se->Result;
}
