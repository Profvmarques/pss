...
$idtipo_segmento = mysql_result($rs, 0, 'idtipo_segmento');
    ...
    $se->consultar("SELECT v.idunidade_escolar, ue.descricao, v.totalvagas,v.remanescente from vagas v inner join unidade_escolar ue on(v.idunidade_escolar= ue.idunidade_escolar) where v.idunidade_escolar NOT IN(66,164,1,2,67,125,180) and v.idsegmento_escolar=" . $_POST['idsegmento_escolar'] . " and v.totalvagas > 0 and v.remanescente > 0 group by v.idunidade_escolar order by ue.descricao;");
    $linha = $se->Linha;
    $resultado = $se->Result;
