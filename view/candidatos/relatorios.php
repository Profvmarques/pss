...
function Processo($Processo) {
 ...
 inner join unidade_escolar ue on(ue.idunidade_escolar = u.idunidade_escolar) where i.idcandidatos=" . $_GET['id']);
			...
			for ($i = 0; $i < $linha; $i++) {
				...
				$array[$i]['dtnasc'] = $util->formatoDataDMY(@mysql_result($rs, $i, 'c.nascimento'));
				$array[$i]['dtinscricao']=@mysql_result($rs, $i, 'i.dtreg');
				$array[$i]['unidade1'] = $candidatos->verificarUnidade(@mysql_result($rs, $i, 'i.idunidade_escolar'));
				$array[$i]['unidade2'] = $candidatos->verificarUnidade(@mysql_result($rs, $i, 'i.idunidade_escolar2'));
				$array[$i]['unidade3'] = $candidatos->verificarUnidade(@mysql_result($rs, $i, 'i.idunidade_escolar3'));
				$array[$i]['unidade4'] = $candidatos->verificarUnidade(@mysql_result($rs, $i, 'i.idunidade_escolar4'));
				$array[$i]['unidade5'] = $candidatos->verificarUnidade(@mysql_result($rs, $i, 'i.idunidade_escolar5'));
				$array[$i]['tipoEnsino'] = @mysql_result($rs,$i,'ts.idtipo_segmento');
...
inscricao.classificacao, candidatos.nome");
			...
			case 'listaCandidatoEd_especial':
  ...
  inner join distrito d on(ue.iddistrito = d.iddistrito) 
  inner join tiposegmento ts on(se.idtipo_segmento = ts.idtipo_segmento)
  where i.etapa='EDUCACAO ESPECIAL'
  group by d.descricao,i.idinscricao order by d.descricao,se.descricao,ue.descricao,i.idinscricao,c.nome;");
            $linha = $candidatos->Linha; //and i.idsegmento_escolar <=21
            $rs = $candidatos->Result;
...
//$array[0]['flag'] = 1;
			...
			case 'listaCandidatoEd_infantil':
            ini_set('memory_limit', '89000M');
