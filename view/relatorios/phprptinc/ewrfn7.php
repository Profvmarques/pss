...
// Functions for PHP Report Maker 7
// (C) 2007-2013 e.World Technology Limited
...
function CurrentProjectID() {
	...
	return "{3F4913F1-1041-46B4-AE4E-992053E3F39C}";
}
...
class crLanguage {
	...
	function LoadFileList() {
		...
		if (is_array($EWR_LANGUAGE_FILE)) {
			...
			for ($i = 0; $i < $cnt; $i++)
				$EWR_LANGUAGE_FILE[$i][1] = $this->LoadFileDesc(EWR_LANGUAGE_FOLDER . $EWR_LANGUAGE_FILE[$i][2]);
		}
	}
	...
	function Load($id) {
		...
		global $EWR_DEFAULT_CURRENCY_SYMBOL, $EWR_DEFAULT_POSITIVE_SIGN, $EWR_DEFAULT_NEGATIVE_SIGN, $EWR_DEFAULT_FRAC_DIGITS;
		global $EWR_DEFAULT_P_CS_PRECEDES, $EWR_DEFAULT_P_SEP_BY_SPACE, $EWR_DEFAULT_N_CS_PRECEDES, $EWR_DEFAULT_N_SEP_BY_SPACE, $EWR_DEFAULT_P_SIGN_POSN, $EWR_DEFAULT_N_SIGN_POSN;
		global $EWR_DEFAULT_LOCALE;
		global $EWR_DEFAULT_TIME_ZONE;
		...
		if (EWR_USE_DOM_XML) {
			...
			if (is_array(@$_SESSION[EWR_PROJECT_VAR . "_" . $sFileName])) {
				$this->Phrases = $_SESSION[EWR_PROJECT_VAR . "_" . $sFileName];
			} else {
		...
		if (is_array($EWR_LANGUAGE_FILE)) {
			...
			for ($i = 0; $i < $cnt; $i++)
				if ($EWR_LANGUAGE_FILE[$i][0] == $Id) {
					return EWR_LANGUAGE_FOLDER . $EWR_LANGUAGE_FILE[$i][2];
			}
		}
		return "";
	}
	...
	function Phrase($Id) {
		...
		if ($ImageUrl <> "") {
			...
			$ImageHeight = ($ImageHeight <> "") ? " height: " . $ImageHeight . "px;" : "";
			return "<img data-phrase=\"" . $Id . "\" src=\"" . ewr_HtmlEncode($ImageUrl) . "\" style=\"border: 0;" . $ImageWidth . $ImageHeight . "\" alt=\"" . ewr_HtmlEncode($Text) . "\" title=\"" . ewr_HtmlEncode($Text) . "\">";
		} else {
			return $Text;
		}
	}
...
class crBreadcrumb {
	...
	function __construct() {
		...
		$this->Links[] = array("home", "HomePage", "index.php", "", FALSE); // Home
		$this->Divider = $ReportLanguage->Phrase("BreadcrumbDivider");
	}
	...
	// Add breadcrumb
	function Add($pageid, $pagetitle, $pageurl, $table = "", $current = FALSE) {
		...
		if (is_array($this->SessionLinks)) {
			...
			for ($i = 0; $i < $cnt; $i++) {
				@list($id, $title, $url, $tbl, $cur) = $this->SessionLinks[$i];
				if (in_array($tbl, $mastertable)) {
					if ($url == $pageurl)
						break;
					if (!$this->Exists($id, $tbl, $url))
						$this->Links[] = array($id, $title, $url, $tbl, FALSE);
				}
			}
		}
		...
		if (!$this->Exists($pageid, $table, $pageurl))
			$this->Links[] = array($pageid, $pagetitle, $pageurl, $table, $current);
	...
	function Render() {
		...
		if (is_array($this->Links)) {
			$cnt = count($this->Links);
			for ($i = 0; $i < $cnt; $i++) {
				list($id, $title, $url, $table, $cur) = $this->Links[$i];
				if ($i < $cnt - 1) {
					$nav .= "<li>";
				} else {
					$nav .= "<li class=\"active\">";
				...
				$title = $this->LanguagePhrase($title, $table, $cur);
				if ($url <> "")
...
function ewr_DrillDownJs($url, $id, $hdr, $usepanel = TRUE, $objid = "", $event = TRUE) {
	if (trim($url) == "") {
		...
		if ($usepanel) {
			...
			if ($event)
				return "ewr_ShowDrillDown(event, " . $obj . ", '" . ewr_JsEncode(ewr_HtmlEncode($url)) . "', '" . ewr_JsEncode($id) . "', '" . ewr_JsEncode($hdr) . "'); return false;";
			else
				return "ewr_ShowDrillDown(null, " . $obj . ", '" . ewr_JsEncode($url) . "', '" . ewr_JsEncode($id) . "', '" . ewr_JsEncode($hdr) . "');";
		} else {
			$url = str_replace("?d=1&", "?d=2&", $url); // Change d parameter to 2
...
class crChart {
	...
	function WriteChartHeader() {
		...
		$chartElement = ($this->ChartType == 20 || (EWR_FUSIONCHARTS_FREE && $this->ChartType <> 21 && $this->ChartType <> 22)) ? 'graph' : 'chart';
...
class crMenu {
	...
	function AddMenuItem($id, $text, $url, $parentid = -1, $src = "", $allowed = TRUE, $grouptitle = FALSE, $customurl = FALSE) {
		$item = new crMenuItem($id, $text, $url, $parentid, $src, $allowed, $grouptitle, $customurl);
	...
	function Render($ret = FALSE) {
		...
		if (!$this->RenderMenu() && EWR_MENUBAR_BRAND == "")
			return;
		if (!$this->IsMobile) {
			if ($this->IsRoot) {
				$str = "<ul";
				...
				$str .= " class=\"" . EWR_MENU_CLASSNAME . "\">\n";
			} else {
				$str = "<ul class=\"" . EWR_SUBMENU_CLASSNAME . "\">\n";
			}
		} else {
			$str = "";
		}
		...
		foreach ($this->ItemData as $item) {
			if ($this->RenderItem($item)) {
				...
				if (!$this->IsMobile && $gtitle && ($gcnt >= 1 || $this->IsRoot)) // Add divider for previous group
					$str .= "<li class=\"" . EWR_MENU_DIVIDER_CLASSNAME . "\"></li>\n";
				if ($item->GroupTitle && (!$this->IsRoot || !EWR_MENU_ROOT_GROUP_TITLE_AS_SUBMENU)) { // Group title
					$gtitle = TRUE;
					$gcnt += 1;
					if (strval($item->Text) <> "") {
						if ($this->IsMobile)
					...
					if (!is_null($item->SubMenu)) {
						foreach ($item->SubMenu->ItemData as $subitem) {
							$liclass = !is_null($subitem->SubMenu) ? EWR_SUBMENU_ITEM_CLASSNAME : "";
							$aclass = "";
							if (!$subitem->IsCustomUrl && ewr_CurrentPage() == ewr_GetPageName($subitem->Url) || $subitem->IsCustomUrl && $cururl == $subitem->Url)
								ewr_AppendClass($liclass, EWR_MENU_ACTIVE_ITEM_CLASS);
					...
					$gtitle = FALSE;
					$liclass = !is_null($item->SubMenu) ? ($this->IsRoot ? EWR_MENU_ITEM_CLASSNAME : EWR_SUBMENU_ITEM_CLASSNAME) : "";
					$aclass = "";
					if (!$item->IsCustomUrl && ewr_CurrentPage() == ewr_GetPageName($item->Url) || $item->IsCustomUrl && $cururl == $item->Url) {
						if ($this->IsRoot)
							ewr_AppendClass($liclass, EWR_MENU_ACTIVE_ITEM_CLASS);
						else
							ewr_AppendClass($liclass, EWR_SUBMENU_ACTIVE_ITEM_CLASS);
					}
		...
		if ($this->IsMobile) {
			...
			if (EWR_MENUBAR_BRAND <> "") {
				...
				$str = "<a class=\"brand\" href=\"" . ewr_HtmlEncode($brandhref) . "\">" . EWR_MENUBAR_BRAND . "</a>" . $str;
			}
			if (EWR_MENUBAR_CLASSNAME <> "" && EWR_MENUBAR_INNER_CLASSNAME <> "")
				$str = "<div class=\"" . EWR_MENUBAR_CLASSNAME . "\"><div class=\"" . EWR_MENUBAR_INNER_CLASSNAME . "\">" . $str . "</div></div>";
		} else {
			$str .= "</ul>\n";
		}
...
class crMenuItem {
	...
	function Render($aclass = "", $liclass = "", $mobile = FALSE) {
		...
		$text = $this->Text;
		if (!is_null($this->SubMenu) && $this->ParentId == -1 && EWR_SUBMENU_DROPDOWN_IMAGE <> "")
			$text .= EWR_SUBMENU_DROPDOWN_IMAGE;
		...
		// Create <LI>
		return ewr_HtmlElement("li", array("class" => $liclass), $innerhtml);
	}
}
	...
	function RenderButtonGroup($body) {
		...
		if (preg_match_all('/<a([^>]*)>([\s\S]*?)<\/a\s*>/i', $body, $matches, PREG_SET_ORDER)) {
			foreach ($matches as $match) {
				...
				$caption = $match[2];
				ewr_PrependClass($class, 'btn'); // Prepend button classes
				if ($this->ButtonClass <> "")
...
class crListOption {
	...
	function Render($Part, $ColSpan = 1) {
		...
		$res = ewr_HtmlElement($this->Parent->Tag, $attrs, $res);
		return $res;
	}
...
class crAdvancedSecurity {
	...
	function AutoLogin() {
		if (@$_COOKIE[EWR_PROJECT_VAR]['AutoLogin'] == "autologin") {
			$usr = TEAdecrypt(@$_COOKIE[EWR_PROJECT_VAR]['Username'], EWR_RANDOM_KEY);
			$pwd = TEAdecrypt(@$_COOKIE[EWR_PROJECT_VAR]['Password'], EWR_RANDOM_KEY);
...
// Add message
function ewr_AddMessage(&$msg, $msgtoadd, $sep = "<p>&nbsp;</p>") {
	if (strval($msgtoadd) <> "") {
		if (strval($msg) <> "")
			$msg .= $sep;
		$msg .= $msgtoadd;
...
function ewr_ErrorFn($DbType, $ErrorType, $ErrorNo, $ErrorMsg, $Param1, $Param2, $Object) {
	...
	$_SESSION[EWR_SESSION_MESSAGE] = $msg;
}
...
function TEAencrypt($str, $key = EWR_RANDOM_KEY) {
	if ($str == "") {
		return "";
	}
	$v = str2long($str, true);
	...
	return ewr_UrlEncode(long2str($v, false));
}

function TEAdecrypt($str, $key = EWR_RANDOM_KEY) {
	$str = ewr_UrlDecode($str);
	if ($str == "") {
		return "";
	}
...
function ewr_UrlEncode($string) {
	$data = base64_encode($string);
	return str_replace(array('+','/','='), array('-','_','.'), $data);
}

function ewr_UrlDecode($string) {
...
function ewr_SendEmail($sFrEmail, $sToEmail, $sCcEmail, $sBccEmail, $sSubject, $sMail, $sFormat, $sCharset, $sSmtpSecure = "", $sAttachmentFileName = "", $sAttachmentContent = "", $arImages = array()) {
...
function ewr_LoadEmailCount() {
	...
	if (EWR_EMAIL_WRITE_LOG) {
		...
		// Load from log file
			...
			$randomkey = TEAencrypt(date("Ymd"), EWR_RANDOM_KEY);
...
function ewr_DebugMsg() {
	global $gsDebugMsg;
	return ($gsDebugMsg <> "") ? "<div>" . $gsDebugMsg . "</div>" : "";
}
...
function ewr_IsMobile() {
	global $MobileDetect, $EWR_USE_MOBILE_MENU;
	if (!isset($MobileDetect))
		$MobileDetect = new Mobile_Detect;
	return $EWR_USE_MOBILE_MENU && $MobileDetect->isMobile();
}
...
// Get jQuery host
function ewr_jQueryHost($mobile) {
...
function ewr_jQueryFile($f) {
	...
	$v = $m ? $mver : $ver;
	return str_replace("%v", $v, ewr_jQueryHost($m) . $f);
}
...
function ewr_ComparePassword($pwd, $input) {
	...
	if (EWR_CASE_SENSITIVE_PASSWORD) {
		...
		if (EWR_ENCRYPTED_PASSWORD) {
			return ($pwd == ewr_EncryptPassword(strtolower($input), @$salt));
		} else {
...
function ewr_DateFilterString($FldExpr, $FldOpr, $FldVal, $FldType) {
	if ($FldOpr == "Year" && $FldVal <> "") { // Year filter
		return str_replace("%s", $FldExpr, "YEAR(%s)") . " = " . $FldVal;
	} else {
...
function ewr_TmpImageLnk($file, $lnktype = "") {
	...
	if ($lnktype == "email" || $lnktype == "cid") {
		...
		return EWR_UPLOAD_DEST_PATH . $file;
	}
}
?>
<?php
