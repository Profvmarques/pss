...
where se.idtipo_segmento=" . $_POST['idtipo_segmento'] . " and v.ideventos=" . $_POST['ideventos'] . " and v.idturno=" . $_POST['idturno'] . " and v.idunidade_escolar=" . $_POST['idunidade_escolar'] . " and v.tipovaga='" . $_POST['tipovaga'] . "';");
        ...
        case 'atualizaVagasPorUnidadesIndividual':

            global $linha;

            global $rs;

            $vagas->consultar("select * from vagas v inner join segmentoescolar se on(v.idsegmento_escolar=se.idsegmento_escolar) 
inner join unidade_escolar ue on(v.idunidade_escolar=ue.idunidade_escolar) 
where se.idsegmento_escolar not in(20,21) and v.idunidade_escolar=".$_SESSION['idunidade_escolar']." and tipovaga='INSCRICAO_FASE2' order by se.ordem");
            $linha = $vagas->Linha;
            $rs = $vagas->Result;
            ...
            if ($_POST['ok'] == 'true') {
                ...
                try {
                    ...
                    for ($i =0; $i <$linha; $i++) {
                        ...
                        $ordem=$_POST['vagas'.mysql_result($rs,$i,'se.ordem')];                        
                        $vagas->atualizaQuadroVagasIndividual($_SESSION['idunidade_escolar'], $idsegmento_escolar,"INSCRICAO_FASE2", $ordem);
                    }

                    $vagas->consultar("COMMIT");
                    $util->msgbox('REGISTRO SALVO COM SUCESSO!');
                    $pg = "default.php?pg=" . base64_encode('view/publico/incluir_unidade_escolar.php') . "&form=" . base64_encode('Painel');
