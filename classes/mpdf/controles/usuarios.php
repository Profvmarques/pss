            ...
            if ($_POST['ok'] == 'true') {
               ...
               if($usuarios->Achou == 'nao') { 
                try {
                    ...
                    $idperfil = 7;
                    $usuarios->consultar("BEGIN");                    
                    $usuarios->incluir($_POST['nome'], $_POST['login'], base64_encode($_POST['login']), $_POST['email'], $idperfil, 137, 'ATIVO');
                    $usuarios->consultar("select * from perfil where idperfil=" . $idperfil);
                    $linha = $usuarios->Linha;
                    $rs = $usuarios->Result;
                    $perfil = mysql_result($rs, 0, 'descricao');
                    $usuarios->EnviarEmail($_POST['login'], $_POST['login'], $_POST['email'], $perfil);
                    $usuarios->consultar("COMMIT");
