...
on(prof.idpessoas = mp.idpessoas) order by p.nome, mp.carga_horaria, mp.matriculaprofessor");
            $linha7 = $turma->Linha;
            $rs7 = $turma->Result;
...
inner join grade_horario gh on(t.idgrade_horario = gh.idgrade_horario) inner join componentecurricular cc on(t.sigladisciplina=cc.sigladisciplina)
 where t.siglacurso='" . $_POST['siglacurso'] . "' and t.idperiodoletivo=" . $_POST['idperiodoletivo'] . " and t.idturno=" . $_POST['idturno'] . " group by t.idturma order by t.idturma,gh.descricao,turno.descricao, d.descricao, st.descricao");
                ...
                for ($i = 0; $i < $linha3; $i++) {
                    $idturma = mysql_result($rs3, $i, 't.idturma');
                    $turma->consultar("select * from turma t inner join periodoletivo pl on(t.idperiodoletivo = pl.idperiodoletivo) inner join situacaoturma st 
on(t.idsituacaoturma = st.idsituacaoturma) INNER join turno on(t.idturno=turno.idturno) inner join disciplinas d 
on(t.sigladisciplina=d.sigladisciplina) left join matriculaprofessor mp on(t.matriculaprofessor = mp.matriculaprofessor) 
left join professor prof on(mp.idpessoas = prof.idpessoas) left join pessoas p on(prof.idpessoas=p.idpessoas)
inner JOIN componentecurricular cc on(cc.sigladisciplina = d.sigladisciplina)
inner join grade_horario gh on(t.idgrade_horario = gh.idgrade_horario) 
 where t.idturma='" . $idturma . "' and t.siglacurso='" . $_POST['siglacurso'] . "' and t.idperiodoletivo=" . $_POST['idperiodoletivo'] . " and t.idturno=" . $_POST['idturno'] . " "
                            . "order by t.sigladisciplina, d.descricao");
                    $linha0 = $turma->Linha;
                    $rs0 = $turma->Result;

                    for ($a = 0; $a < $linha0; $a++) {
                        
                        $arr[$count]['idturma'] = mysql_result($rs0, $a, 't.idturma');
                        $arr[$count]['sigla'] = mysql_result($rs0, $a, 't.sigladisciplina');
                        $arr[$count]['disciplina'] = mysql_result($rs0, $a, 'd.descricao');
                        $arr[$count]['periodo'] = mysql_result($rs0, $a, 'cc.periodo');
                        $arr[$count]['professor'] = @mysql_result($rs0, $a, 'p.nome');
                        $arr[$count]['matriculaprof'] = @mysql_result($rs0, $a, 't.matriculaprofessor');
                        $count++;
                    }
                }
...
left join matriculaprofessor mp on(t.matriculaprofessor = mp.matriculaprofessor) 
inner join grade_horario gh on(t.idgrade_horario = gh.idgrade_horario)
 where t.idmatriz='" . $_GET['idmatriz'] . "' and t.idturma='" . $_GET['idturmas'] . "' and t.siglacurso='" . $_GET['siglacurso'] . "' and t.idperiodoletivo=" . $_GET['idperiodoletivo'] . " and t.idturno=" . $_GET['idturno'] . " and t.idgrade_horario=" . $_GET['idgrade_horario'] . " order by gh.descricao,turno.descricao, d.descricao, st.descricao");
...
on(prof.idpessoas = mp.idpessoas) order by p.nome, mp.carga_horaria, mp.matriculaprofessor");
            ...
            if ($_POST['ok'] == 'true') {
                try {
                    ...
                    $turma->consultar("BEGIN");
                    $turma->AlterarTurma($_GET['idturmas'], $_GET['idmatriz'], $_GET['siglacurso'],  $_GET['idperiodoletivo'], $_GET['idgrade_horario'], $_POST['idturma'], $_POST['siglacurso'], $_POST['idmatriz'], $_POST['idgrade_horario'], $_POST['idperiodoletivo'], $_POST['idturno'], $_POST['idsituacaoturma'], $_POST['qtdetotal']);
                    $turma->consultar("COMMIT");
...
on(prof.idpessoas = mp.idpessoas) order by mp.matriculaprofessor, p.nome");
            ...
            $turma->consultar("select * from temposemanal");
            $linha8 = $turma->Linha;
            $rs8 = $turma->Result;
